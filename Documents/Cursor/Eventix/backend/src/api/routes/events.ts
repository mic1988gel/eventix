...
router.post("/", (req, res) => {
  ...
  `);
  const r = stmt.run(
    e.title, e.description ?? null, e.category, e.date, e.time, e.address,
    e.latitude ?? null, e.longitude ?? null, e.max_participants ?? 10, e.created_by
  );
...
router.put("/:id", (req, res) => {
  ...
  db.prepare(`
    ...
    WHERE id=?
  `).run(
    e.title ?? null, e.description ?? null, e.category ?? null, e.date ?? null,
    e.time ?? null, e.address ?? null, e.latitude ?? null, e.longitude ?? null,
    e.max_participants ?? null, id
  );
